...
export class CustomLoggerService implements LoggerService {
  ...
  private static instance?: typeof CustomLoggerService | LoggerService =
    CustomLoggerService;
  ...
  static setGlobalPrefix(prefix: string) {
    CustomLoggerService.prefix = prefix;
  }
  ...
  private callFunction(name: LogLevel, message: any, context?: string) {
    ...
    const instance = this.getInstance();
    const func = instance && (instance as typeof CustomLoggerService)[name];
    // tslint:disable-next-line: no-unused-expression
    func &&
      func.call(
        instance,
  ...
  private getInstance(): typeof CustomLoggerService | LoggerService {
    const { instance } = CustomLoggerService;
    return instance === this ? CustomLoggerService : instance;
  }

  private isLogLevelEnabled(level: LogLevel): boolean {
    return CustomLoggerService.logLevels.includes(level);
  }
  ...
  private static printMessage(
    ...
    process.stdout.write(
      color(`[${this.prefix || 'Nest'}] [${name}] ${process.pid}   - `),
    );
  ...
  private static printTimestamp(isTimeDiffEnabled?: boolean) {
    const includeTimestamp =
      CustomLoggerService.lastTimestamp && isTimeDiffEnabled;
    if (includeTimestamp) {
      process.stdout.write(
        yellow(` +${Date.now() - CustomLoggerService.lastTimestamp}ms`),
      );
    }
